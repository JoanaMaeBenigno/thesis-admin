...
import { postVideo } from "@/services/file";
...
export default function AddVideoPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await postVideo(JSON.stringify(payload));

      setModalMessage("Video saved successfully!");
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-sm w-full shadow-lg text-center space-y-4">
            ...
            <button
              onClick={() => {
                if (modalSuccess) router.push("/dashboard/video");
              }}
